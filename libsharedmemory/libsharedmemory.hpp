...
inline std::string errorToString(Error error) {
    switch (error) {
        ...
        case Error::MappingFailed:
            return "Error: Mapping failed";
        case Error::OpeningFailed:
...
inline Memory::Memory(const std::string& path, std::size_t size, bool persist) : _path(path), _size(size), _persist(persist)
{
    if (_persist)
    {
...
public:
    ...
    {
        memoryErrorStatus = _memory.open();
        if (memoryErrorStatus != Error::OK)
        {
            push_error("Shared memory segment could not be opened. (reason: " + errorToString(memoryErrorStatus) + ")");
        }else{
...
private:
    ...
    Memory _memory;
    Error memoryErrorStatus = Error::OK;
...
public:
    ...
    SharedMemoryWriteStream(const std::string& name, const std::size_t bufferSize, const bool isPersistent):
        _memory(name, bufferSize, isPersistent)
    {
        memoryErrorStatus = _memory.create();

        if (memoryErrorStatus != Error::OK)
        {
            push_error("Shared memory segment could not be created. (reason: " + errorToString(memoryErrorStatus) + ")");
        }else{
            auto memory = static_cast<char*>(_memory.data());
            memory[0] = 0;
